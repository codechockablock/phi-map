...
| The behaviour being measured — decision determined by option/catalogue order | **Published**, but verified less firmly than the rest of this table. Position bias in option selection is treated as settled by work building mitigations for it ([SCOPE](https://arxiv.org/abs/2507.18182): LLMs "can achieve inflated scores on multiple-choice tasks by exploiting inherent biases in option positions"). The tool-ordering figure of 41%→27% quoted here on first pass came from a search summary and could not be verified — that PDF yields no extractable text. Treat the phenomenon as solid and that number as unsourced. |
