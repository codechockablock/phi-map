"""Valence position-contamination check -- R3 CORRECTED COPY.

================================ STATUS ======================================
POST-HOC. NO PRE-REGISTERED STATUS. Anything this file produces is post-hoc
until a future pre-registration adopts it BY NAME and states its criteria first.

Forked from `valence_position_check.py` (as-run at commit d6aac3e, blob
1295f57979501643a14f16db0df9a6a3d40984aa) which is preserved unedited and still
holds the code that produced every number in `docs/step4-results-2026-07-30.md`.

    -> valence_position_check.py      (as-run, pre-registered, defect intact)

THE ONE DIFFERENCE: `prior_rating` now reads the last NON-PAD position via the
shared helper in `measure_primitives.py`, instead of `logits[:, -1, :]` (the
last position of the padded batch). Everything else is unchanged.

Consequence: running this produces DIFFERENT P1 numbers than the results doc
reports, by design. That is not a discrepancy to reconcile -- the doc's P1
figures are retracted (results section 9.1), not superseded.
==============================================================================
